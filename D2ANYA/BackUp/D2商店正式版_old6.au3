...
$bindmac = 1 ;绑定机器
$bindacc = 0 ;绑定帐号
$bindlimitCount = 1 ;  绑定次数  ---可以告知给用户的测试次数
$limitRound = 5;持续挂机次数
...
;~ 			EndIf
		...
		Case $istrade = 0 And findrightdoor() = True And findanya() = True ;左边有红门，有安亚,找安亚交易
			clickrightdoor()
			clickAnya()
			Sleep(100)
			clickTrade()
		...
		Case $istrade = 1 And findrightdoor() = True  ;flag=1 ,  右边有红门，则进右边红门，设置flag为0
			...
			Sleep(100)
				...
				Case $istrade = 1 And findrightdoor() = True  ;flag=1 ,  左边有红门，进左边红门，设置flag为0
			clickrightdoor()
			Sleep(100)
