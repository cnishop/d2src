...
;~
				...
				Do
					$inMiddle = Middle3c()
					If $inMiddle <> -1 Then
						MouseClick("left", $inMiddle[0] - Random(1, 80), $inMiddle[1] + Random(50, 80), Random(1, 3, 1))
						Sleep(1000)
						;每次移动一下方位
					...
					Else; 如果没有找到，就标记下
						$find1 = $find1 + 1
					EndIf
;### Tidy Error -> "endif" is closing previous "do" on line 261
				EndIf
;### Tidy Error -> "until" is closing previous "while" on line 248
...
;### Tidy Error -> "wend" is closing previous "if" on line 243
		WEnd
;### Tidy Error -> "else" is closing previous "do" on line 241
	Else;即一个红色点都没找到的话，再次查找，如果累计不满足条件，就退出
		
		$FailsCount = $FailsCount + 1
		$dif = TimerDiff($beginAttackTime)
;### Tidy Error -> "endif" is closing previous "do" on line 241
	EndIf
...
;### Tidy Error -> "until" is closing previous "func" on line 217
