...
;~
				...
				Do
					$inMiddle = Middle3c()
					If $inMiddle <> -1 Then
						MouseClick("left", $inMiddle[0] - 80, $inMiddle[1] + 80, Random(1, 3, 1))
						Sleep(1000)
						;每次移动一下方位
						...
						If $tp_Pix[0] >= 0 And $tp_Pix[1] >= 0 Then
							...
							If bhfire(8, $tp_Pix[0], $tp_Pix[1]) = 0 Then
								TrayTip("", "攻击失败", 1, 16)
								ExitLoop
							EndIf
							$firstAttack = 1
						...
						EndIf
					EndIf
;### Tidy Error -> "until" is closing previous "if" on line 272
					Until $inMiddle <> -1
					TrayTip("", "换地点攻击", 1, 16)
					Sleep(2000)
...
;### Tidy Error -> "wend" is closing previous "do" on line 270
				WEnd
;### Tidy Error -> "else" is closing previous "while" on line 258
			Else;即一个红色点都没找到的话，再次查找，如果累计不满足条件，就退出
				
				$FailsCount = $FailsCount + 1
				$dif = TimerDiff($beginAttackTime)
;### Tidy Error -> "endif" is closing previous "while" on line 258
			EndIf
...
;### Tidy Error -> "until" is closing previous "if" on line 253
		Until $FailsCount >= 6 Or $dif >= $delayTime ;搜索失败次数大于5次 或者 总时间超过了50秒
		Return 0
;### Tidy Error -> "endfunc" is closing previous "do" on line 251
