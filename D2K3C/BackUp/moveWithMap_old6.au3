...
;检查包裹，得出物品，得出剩余数量
;移动
;再左边有点击下，如果满了，会自动移动回来
;最后40格全部移动后，
;检查数量，是否和移动前一样
Func mapmove()
	mapInitialBagCheck()
	;----------先设置默认的
	$firstbagCount = mapgetbagLocation()
	;---------
	...
	mapInitialBagCheck() ;在初始化，得出最后剩余数量
...
Func mapgetbagLocation()
	...
	mapbagfullcompare() ; 先初始化空包裹下的格子的颜色
...
;~ 				$itembaglocationxy[0][1] = $j ;记录物品的初始位置的 坐标
				...
				If rightmoveNew() == 0 Then
					ExitLoop 2
				EndIf
					;ExitLoop 2
				EndIf
;### Tidy Error -> "next" is closing previous "if" on line 357
			Next
		Next
		
;### Tidy Error -> "endfunc" is closing previous "for" on line 353
...
;### Tidy Error -> "func" cannot be inside any IF/Do/While/For/Case/Func statement.
	Func rightmoveNew() ;通过判断 仓库整个画面的模式
		...
		MouseClick("right", 430 + Random(1, 50, 1), 330 + Random(1, 80, 1), Random(1, 3, 1))
		Sleep(50)
		$last = PixelChecksum(160, 150, 320, 370, $title)
		MouseMove(350 + Random(1, 10, 1), 300 + Random(1, 10, 1), 0)
		If $first = $last Then ;如果相等，表示这个物品没放得进去
			
			$firstCount = getbagLocation() ;检查下这个物品的格子数，如果1格，那箱子满了，后续就不用操作了
			MouseClick("right", 290 + Random(1, 10, 1), 500 + Random(1, 20, 1), Random(1, 3, 1), 0);  此时移动到左边,有点击,防止仓库满了,把东西返回原样,放入包裹
			Sleep(200)
