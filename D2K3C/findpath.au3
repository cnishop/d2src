...
Func goto3cpath($path)
	...
	Select
		...
		Case Else
			...
			MouseClick("left", 600, 450, Random(2, 5, 1))
			Sleep(1500)
			$coord = PixelSearch(400, 200, 790, 560, 0x18FC00, 10, 1, $title)
			If Not @error Then
				MouseClick("left", $coord[0] + 150, $coord[1] - 100, Random(1, 5, 1))
				Sleep(500)
			EndIf

			MouseClick("left", 450, 180, Random(2, 4, 1))
			Sleep(600)

			$coord = PixelSearch(450, 200, 790, 560, 0x18FC00, 10, 1, $title)
			If Not @error Then
				MouseClick("left", $coord[0] + 280, $coord[1] + 100, Random(1, 5, 1))
				Sleep(1000)
			EndIf

			$coord = PixelSearch(450, 300, 790, 560, 0x18FC00, 10, 1, $title)
			If Not @error Then
				MouseClick("left", $coord[0], $coord[1] + 30, Random(1, 5, 1))
				Sleep(1000)
			EndIf
			;MouseClick("left", 600, 280, Random(1, 5, 1)) ;水平方向移动
			MouseMove(600, 280,Random(0,8))
			Sleep(50)
			MouseDown("left")
			Sleep(300)
			MouseUp("left")
			...
			$begin1 = TimerInit()
			Do
				$coord = PixelSearch(400, 200, 720, 400, 0x18FC00, 10, 1, $title)
				If @error Then
					MouseClick("left", 600, 280, Random(1, 5, 1))  
					Sleep(50)
					$findflag = 0
				Else
					MouseClick("left", $coord[0] + 50, $coord[1] + 100, Random(3, 5, 1))
					Sleep(300)
					$findflag = 1
				EndIf
			...
			MouseClick("left", 620, 410, Random(1, 3, 1))
			Sleep(500)

			$coord = PixelSearch(400, 300, 750, 560, 0x18FC00, 10, 1, $title)
			If Not @error Then
				MouseClick("left", $coord[0], $coord[1] + 50, Random(1, 5, 1))
				Sleep(1000)
			EndIf

			MouseClick("left", 600, 170, Random(1, 5, 1))
			Sleep(500)
	...
	Select
		...
		Case Else
			;If isInRoom() Then
				...
				MouseClick("left", 700 + Random(1,20,1), 320 + Random(1,10,1), Random(1, 3, 1))
				Sleep(500)
				CheckMove($Char_CheckMoveDelay)
				$coord = PixelSearch(450, 50, 790, 400, $a3colorBlock, 10, 1, $title)
				If not @error Then
					MouseClick("left", $coord[0] + 100, $coord[1] +100 , Random(1, 5, 1))
					Sleep(500)
				Else
					MouseClick("left", 700, 300, Random(1, 3, 1))
					CheckMove($Char_CheckMoveDelay)
				EndIf

				MouseClick("left", 700, 100, Random(1, 3, 1))
				Sleep(500)

				$findflag = 0
				$begin1 = TimerInit()
				Do
					$coord = PixelSearch(400, 250, 700, 400, $a3colorBlock, 10, 1, $title)
					If @error Then
						MouseClick("left", 700 + $xrd, 100 + $yrd, Random(1, 5, 1))
						Sleep(50)
						$findflag = 0
					Else
						MouseClick("left", $coord[0] + 50, $coord[1] - 50, Random(3, 5, 1))
						Sleep(300)
						$findflag = 1
					EndIf
					$dif = TimerDiff($begin1)
...
Func a3omousToWp()
	...
	Select
		...
		Case Else
			TrayTip("", "路径2...", 1, 16)
			MouseClick("left", 760, 300, Random(2, 4, 1))
			Sleep(300)
			CheckMove($Char_CheckMoveDelay)
			MouseClick("left", 760, 290, Random(2, 4, 1))
			Sleep(300)
			MouseClick("left", 600, 250, Random(1, 5, 1))
			CheckMove($Char_CheckMoveDelay)
			$coord = PixelSearch(350, 200, 790, 350, $a3colorBlock, 10, 1, $title)
			If Not @error Then
				MouseClick("left", $coord[0] - 100, $coord[1], Random(3, 5, 1))
				Sleep(500)
			EndIf
			CheckMove($Char_CheckMoveDelay)
