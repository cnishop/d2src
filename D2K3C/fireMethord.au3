...
;~
		...
		Else;即一个红色点都没找到的话，再次查找，如果累计不满足条件，就退出
			$FailsCount = $FailsCount + 1
			TrayTip("", "没有目标，再次监测 " & $FailsCount, 1, 16)
			;Sleep(1)
		EndIf
		
		$dif = TimerDiff($beginAttackTime)
	Until $FailsCount >= 5 Or $dif >= $delayTime ;搜索失败次数大于5次 或者 总时间超过了50秒
	$inMiddle = Middle3c() ;再次回到中间位置
	If $inMiddle <> -1 Then
		MouseClick("left", $inMiddle[0] - 80, $inMiddle[1] + Random(50, 80), Random(1, 3, 1))
		Sleep(1500)
	EndIf
...
Func fireMonsterByBlockWithoutMap($XBS_Start_PARM, $YBS_Start_PARM, $XBS_Stop_PARM, $YBS_Stop_PARM, $kpmode, $delayTime) ; $kpmode =1 ,表示只打右上角 $delayTime 限定一个时间
	$findrecround = 0 ;  中间往四周找的次数
	$findfourround = 0 ;四个方位找怪
