...
Func mapmove()
	...
	If $firstbagCount = $lastbagCount Then
		...
		TrayTip("", "箱子装不下了", 1, 16)
		Sleep(500)
		If  $firstCount <= 6 Then ;如果是第一次循环就放不下了，表示满了
...
Func rightmoveNew() ;通过判断 仓库整个画面的模式
	...
	If $first = $last Then ;如果相等，表示这个物品没放得进去
		...
		$firstCount = mapgetbagLocation() ;检查下这个物品的格子数，如果1格，那箱子满了，后续就不用操作了
		MouseClick("right", 290 + Random(1, 10, 1), 500 + Random(1, 20, 1), Random(1, 3, 1), 0);  此时移动到左边,有点击,防止仓库满了,把东西返回原样,放入包裹
		Sleep(200)
		$lastCount = mapgetbagLocation()
		$itemSize =   $firstCount -$lastCount
