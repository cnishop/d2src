...
Func runGame()
	...
	If $round >= 80 Then ;测试版
		writelog("系统强制下线，看完挂机经验再挂机")
		MsgBox(4096, " ..... 温馨提示 .........", "请休息下再挂机")
		Exit 0
	EndIf
...
;~ 							EndIf
					...
					Case 3 ;杀山克
						...
						Do
							If isInRoom() Then
								$coord = PixelSearch(10, 300, 700, 400, $biaojicolor1, 10, 1, $title) ;0x118FB01
								If @error Then
									TrayTip("", "now tp ...", 1, 16)
									$i = Random(1, 2, 1)
									If $i = 1 Then
						...
						If $topleft = True And $bottomright = True Then ;;上下都有标记位，到达指定地点，可以杀怪
							...
							$coord = PixelSearch(400, 300, 750, 540, $biaojicolor1, 10, 1, $title)
							If Not @error Then
								$bottomright = True
							...
							EndIf
							$coord = PixelSearch(400, 10, 750, 540, $biaojicolor1, 10, 1, $title)
							If Not @error Then
								$bottomright = True
								;TrayTip("", $coord[0] & "  " & $coord[1], 1, 16)
...
Func astisdead()
	...
	If isInRoom() Then
		...
		If CheckpetStatus() = 0 Then ; And isInTown() = True
			Return True
...
Func child1()
	...
	While ProcessExists($gi_CoProcParent)
		...
		If isInRoom() = True And isOutTown() = False And $petStatus = 0 And $needResumePetFlag = 0 Then ;站城内部退出
