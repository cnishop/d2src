...
Func runGame()
	...
	If $round >= 300 Then ;测试版
		writelog("系统强制下线，看完挂机经验再挂机")
			...
			Case Else ;比如kp，k山克等
				...
				Switch $killQuery ;
					...
					Case 2 ;矫正者 冰免  jzz
						...
						For $i = 1 To 10 Step 1  ;强制攻击
								nowfire(500 + Random(1, 50, 1), 560 + Random(1, 30, 1))
							Next
...
;~ 							EndIf
							...
							If isInRoom() Then ;TP 到怪物面前
								...
								$begin1 = TimerInit()
								Do
									$coord = PixelSearch(50, 25, 780, 250, $MonsterRed, 10, 1, $title)
									If @error Then
							...
							finditem($parm_picktime)
							If $onlykp <> 1 Then
								$killQuery = 3 ; 把标记位改为其他，就下局就可以k其他怪了
					...
					Case 3 ;杀山克
						...
						;到达第二个小站后，开始判断是否到达
						MouseMove(400 + Random(1, 10), 300 + Random(1, 10), 0)
						$coord1 = PixelSearch(20, 20, 400, 300, $biaojicolor1, 30, 1, $title)
						If Not @error Then
							$topleft = True
							TrayTip("", "top flag", 1, 16)
						...
						If $topleft = True And $bottomright = True Then ;;上下都有标记位，到达指定地点，可以杀怪
							...
							EndIf
							$coord = PixelSearch(420, 200, 780, 500, $biaojicolor1, 20, 1, $title) ;到达最右边一个标志点
							If Not @error Then
								$bottomright = True
