...
$RegGUI = GUICreate(" 暗黑KP挂机程序注册", 283, 157, @DesktopWidth / 2 - 150, @DesktopHeight / 2 - 150)
...
Func _CdKey() ;加密的关键部分 这里是用MD5加密  整个注册机的关键部分就是这里, 如果要自己写加密算法 那就复杂了
	...
	;以下是针对绑定账号的
	If $bindacc = 1 Then
		$Cdkey = StringMid(_MD5($Usrid & $KeyPw), 3, 34) ;MD5加密 没事可以试着破解一下@!!!!!
	EndIf
