...
$acountArray[0] = "stefkid"
$acountArray[1] = "stefkid"
...
$ranLimte = Random(-10, 13, 1) ;最大次数限制的前后范围   比如可以 80-20 或者 80+20你
;$ranLimte = 0
;///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////各种版本
...
Local $ct = 1 ;房间名加1
Local $repeatrejRole = 0 ; 增加一个复活角色的标志，看是否重复
...
Local $buyScrollTownFlag = 0 ;   是否买单个回城卷轴 ，靠 onlykp 控制开关
Local $needbuyScrollTown = 0;  //实际是否需要买回城
If $onlykp = 1 Then
...
Local $MonsterRed = "0xFC2C00"
Local $MonsterRed_hex = "FC2C00"
...
Func runGame()
	...
	If $round >= 500 Then ;测试版
		writelog("持续超过块8个多小时了，注意危险")
	...
	If isInRoom() Then
		Select
			...
			Case isInRoom() And $ckroledead = 1 And roleisdead() ;先判断雇佣斌是否死亡。
				$repeatrejRole = $repeatrejRole + 1 ;
				writelog("角色---第 " & $round & " 局: " & "角色死亡")
				TrayTip("", "尝试第" & $repeatrejRole & " 次复活角色", 1, 16)
				If $repeatrejRole >= 6 Then ;如果 发现重复执行复活的次数大于 10次，表示可能没钱了。
					writelog("角色---第 " & $round & " 局: " & "连续多次次重复复活角色，可能是异常，自动退出。")
					WinClose($title)
					Sleep(1000)
					Exit 0
				EndIf
			...
			Case isInTown() And $parm_imageMode = 2 And $buyScrollTownFlag = 1 And $needbuyScrollTown = 1;---是否买回城卷轴
				TrayTip("", "buy some scroll of town portal", 1, 16)
				;--先看是否有紫药水，有的话，就不购买
				...
				$coord = findScrollownInShop()
				If $coord[0] >= 0 And $coord[1] >= 0 Then
					TrayTip("", "find scroll of town portal.", 1, 16)
					Sleep(100)
					Sleep(10)
					Send("{LSHIFT down}")
					Sleep(50)
					...
					For $i = 0 To 2 Step 1
						Send("{LSHIFT}")
					Next
					$needbuyScrollTown = 0 ;购买成功后，改成 0
				Else
					TrayTip("", "can not find it.", 1, 16)
					Sleep(1000)
				EndIf
			...
			Case Else ;比如kp，k山克等
				...
				Switch $killQuery ;
					Case 1 ;kp
						If isInTown() = False Then ;如果不在城内，那就重新开始
							Return
						EndIf
						;检查是否按到了b
						If isInRoom() And findPointColor(500, 440, "4C4C4C") = True Then
							...
							Sleep(500)
						EndIf
						$armrnd = Random(1, 5, 1) ;加入随即，可放，或者不放装甲技能
						If $armrnd = 1 Then
						...
						If isInTown() And finddoor() Then
							
							TrayTip("", "moving now...", 1, 16)
							movekp() ;路径需要优化，防止同一落脚点
							Sleep(Random(20, 50, 1))
							CheckMove($Char_CheckMoveDelay)
							If isInRoom() = True And isInTown() Then ;如果还在城内
								exitRoom()
								Return
							EndIf
							...
							Else
								If $dingZhiFlag = "109" Then
									fire(2)
								Else
									fireMonsterByBlock(400, 20, 780, 520, 1, 10000) ; 借助mod，锁定红色方框的攻击方式
								EndIf
							EndIf
							;fire("109")
							...
							If $onlykp = 1 Then ;如果109，就不k其他的
								...
								EndIf
							Else
								If ReturnToTownWithMap() = False Then ;如果回城失败，那就退出房间
									$needbuyScrollTown = 1 ;  增加标记位，下把买回城卷轴
									exitRoomWithMap()
								EndIf
					...
					Case 2 ;矫正者 冰免  jzz
						If isInTown() = False Then ;如果不在城内，那就重新开始
							...
							Return
						EndIf
						TrayTip("", "kill other monsters", 1, 16)
						$topleft = False
						$bottomright = False
						
						MouseClick("left", 135 + Random(1, 10), 400 + Random(1, 10), Random(1, 3, 1))
						Sleep(500)
						;CheckMove($Char_CheckMoveDelay)
						MouseClick("left", 250 + Random(1, 10), 400 + Random(1, 10), Random(1, 3, 1))
						CheckMove($Char_CheckMoveDelay)
						MouseClick("left", 380 + Random(1, 10), 350 + Random(1, 10), Random(1, 3, 1))
						CheckMove($Char_CheckMoveDelay)
						$coord = finda5xiaozhan() ;找小站的图片
						If $coord[0] >= 0 And $coord[1] >= 0 Then
							TrayTip("", "go to waypoint", 9, 16)
							Sleep(50)
						...
						Else
							exitRoom()
							TrayTip("", "find waypoint failed..", 9, 16)
							Sleep(1000)
						EndIf
...
;~ 						EndIf
						...
						If $topleft = True Then ;;上下都有标记位，到达指定地点，可以杀怪
							fireMonsterByColor(300, 350, 780, 560, 0)
							Sleep(10)
							If $fire = 1 Then
								Send("{" & $char_tpFire & "}")
							Else
							...
							Sleep(500)
							If isInRoom() Then ;TP 到怪物面前
								$xrd = Random(-2, 2, 1)
								$yrd = Random(-2, 2, 1)
								$findflag = 0 ;a5right ; 从箱子下方找右边第二个方格
								$begin1 = TimerInit()
								Do
									$coord = PixelSearch(50, 25, 780, 200, $MonsterRed, 10, 1, $title)
									If @error Then
										MouseClick("right", 420 + $xrd, 120 + $yrd, Random(1, 5, 1))
										Sleep(300)
										CheckMove($Char_CheckMoveDelay)
										$findflag = 0
									Else
							...
							For $i = 1 To 2 Step 1
								nowfire(420 + Random(1, 50, 1), 80 + Random(1, 30, 1))
							Next
							
							;fireMonsterByBlock(100, 20, 790, 520, 0) ; 借助mod，锁定红色方框的攻击方式
							fireMonsterByColor(20, 20, 780, 560, 0)
							If $fire = 1 Then
								Send("{" & $char_tpFire & "}")
							Else
								Send("{" & $char_tpFire & "}")
							EndIf
							MouseClick("right", 420 + $xrd, 120 + $yrd, 1) ;往前走一步方便捡东西
							Sleep(800)
							TrayTip("", "pick up item.", 1, 16)
							finditem($parm_picktime)
							Sleep(10)
							fireMonsterByColor(10, 10, 780, 540, 0)
							If $onlykp <> 1 Then
								$killQuery = 1 ; 把标记位改为其他，就下局就可以k其他怪了
								If $killQuery = 1 Then ;如果标记位是1，就不打了重新来
									exitRoom()
									Return
								EndIf
							Else
					...
					Case 3 ;杀山克   djsk
						...
						Do
							If isInRoom() Then
								$coord = PixelSearch(5, 300, 700, 500, $biaojicolor1, 10, 1, $title) ;0x118FB01
								If @error Then
									TrayTip("", "now tp ...", 1, 16)
									$i = Random(1, 2, 1)
									If $i = 1 Then
										MouseClick("right", 420 + $xrd, 450 + $yrd, Random(1, 3)) ;50
									Else
										MouseClick("right", 420 + $xrd, 450 + $yrd, Random(1, 3)) ;50
									EndIf
									Sleep(400)
								...
								Else
									$findflag = 1
									MouseClick("right", $coord[0] + 40, $coord[1] + 10, Random(1, 3)) ;50
									Sleep(500)
								EndIf
							Else
								Return
						...
						If $topleft = True And $bottomright = True Then ;;上下都有标记位，到达指定地点，可以杀怪
							...
							$coord = PixelSearch(400, 300, 750, 540, $biaojicolor1, 30, 1, $title)
							If Not @error Then
								;TrayTip("", $coord[0] & "  " & $coord[1], 1, 16)
								MouseMove($coord[0] + Random(40, 45), $coord[1] + Random(20, 25))
								MouseClick("right", Default, Default, 1)
								Sleep(300)
								CheckMove($Char_CheckMoveDelay)
								MouseClick("right", 520, 300, 1)
								Sleep(500)
								CheckMove($Char_CheckMoveDelay)
							Else
								Sleep(200)
							...
							fireMonsterByColor(400, 340, 780, 520, 0) ;打怪
							Sleep(100)
							If $fire = 1 Then
								Send("{" & $char_tpFire & "}")
							ElseIf $fire = 2 Then
							...
							EndIf
							TrayTip("", "入口处", 1, 16)
							$coord = PixelSearch(400, 100, 760, 540, $biaojicolor1, 30, 1, $title)
							If Not @error Then
								$bottomright = True
								TrayTip("", "楼梯上1", 1, 16)
							...
							For $i = 1 To 2 Step 1 ;强制攻击
								TrayTip("", $i, 9, 16)
								nowfire(470 + Random(30, 50, 1), 350 + Random(1, 50, 1))
							Next
							
							fireMonsterByColor(300, 340, 780, 540, 0)
							Sleep(100)
							If $fire = 1 Then
								Send("{" & $char_tpFire & "}")
								Sleep(200)
							...
							EndIf
							$coord = PixelSearch(500, 300, 780, 540, $biaojicolor1, 30, 1, $title)
							If Not @error Then
								$bottomright = True
								TrayTip("", "楼梯上2", 1, 16)
							...
							EndIf
							$coord = PixelSearch(410, 300, 750, 540, $biaojicolor1, 30, 1, $title)
							If Not @error Then
								$bottomright = True
								TrayTip("", "楼梯上3", 1, 16)
								;TrayTip("", $coord[0] & "  " & $coord[1], 1, 16)
							...
							EndIf
							$coord = PixelSearch(500, 300, 780, 540, $biaojicolor1, 30, 1, $title) ;达到楼梯下方的打怪点，可以打怪
							If Not @error Then
								$bottomright = True
								TrayTip("", "楼梯下1", 1, 16)
								;TrayTip("", $coord[0] & "  " & $coord[1], 1, 16)
							...
							fireMonsterByColor(20, 100, 780, 540, 0)
							Sleep(10)
							If $fire = 1 Then
								Send("{" & $char_tpFire & "}")
								Sleep(200)
							...
							EndIf
							$coord = PixelSearch(420, 200, 600, 500, $biaojicolor1, 20, 1, $title) ;到达中间打怪点
							If Not @error Then
								$bottomright = True
								;TrayTip("", $coord[0] & "  " & $coord[1], 1, 16)
								TrayTip("", "out", 1, 16)
							...
							fireMonsterByColor(20, 20, 780, 540, 0)
							Sleep(10)
							If $fire = 1 Then
								Send("{" & $char_tpFire & "}")
								Sleep(100)
							...
							EndIf
							$coord = PixelSearch(420, 200, 780, 500, $biaojicolor1, 30, 1, $title) ;到达最右边一个标志点
							If Not @error Then
								TrayTip("", "out2.", 1, 16)
								TrayTip("", "center of there", 1, 16) ;楼梯下4个色块的中间位置
							...
							Else
								...
								Return
							EndIf
							fireMonsterByColor(20, 320, 780, 540, 0) ;打怪
							Sleep(200)
							;finditembyzone(50, 50, 400, 500, $parm_picktime)
							$coord = PixelSearch(420, 200, 780, 500, $biaojicolor1, 10, 1, $title) ;到达最右边一个标志点
							If Not @error Then
								$bottomright = True
								TrayTip("", $coord[0] & "  " & $coord[1], 1, 16)
								MouseMove($coord[0] + Random(10, 15), $coord[1] + Random(130, 140))
							...
							fireMonsterByColor(20, 20, 780, 550, 0) ;可以打怪
							Sleep(10)
							If $fire = 1 Then
								Send("{" & $char_tpFire & "}")
								Sleep(200)
							...
							EndIf
							MouseClick("right", 250, 150, Random(1, 3, 1))
							Sleep(500)
							MouseClick("right", 250, 150, Random(1, 3, 1))
							Sleep(500)
							fireMonsterByColor(20, 20, 780, 550, 0)
							finditem($parm_picktime)
							
							TrayTip("", "Exit room.", 1, 16)
							exitRoom()
		...
		Case Else
			...
			Switch $i
				Case 1
					Do
						If isOutTown() Then
							$coord = PixelSearch(10, 10, 700, 300, $biaojicolor1, 10, 1, $title) ;0x118FB01
							If @error Then
							...
							EndIf
						Else
							TrayTip("", "Teleport Failed...1", 1, 16)
							Return
						EndIf
				...
				Case 2
					Do
						If isOutTown() Then
							$coord = PixelSearch(10, 10, 700, 300, $biaojicolor1, 20, 2, $title)
							If @error Then
								TrayTip("", "Now Teleport...", 1, 16)
							...
							EndIf
						Else
							TrayTip("", "Teleport Failed...2", 1, 16)
							Return
						EndIf
						$dif = TimerDiff($beginAttackTime)
			...
			$coord = PixelSearch(10, 5, 500, 300, $biaojicolor1, 20, 2, $title) ; 找到左上角的绿色标记后，tp 到它的右边
			If Not @error Then
				TrayTip("", "keep moving 2", 1, 16)
				If $coord[0] + 250 >= 790 Or $coord[1] + 120 >= 580 Then ;判断是否越界，若越界，可能是飞过了
					TrayTip("", "keep moving 2 error", 1, 16)
					Sleep(300 + $parm_speeddelay)
			...
			Do
				;CheckMove($Char_CheckMoveDelay)
				$coord = PixelSearch(10, 5, 700, 300, $biaojicolor1, 30, 1, $title) ;
				If Not @error Then
					$coord1 = PixelSearch(350, 280, 790, 560, $biaojicolor1, 25, 2, $title) ; 看它右下方有没有绿色标记，有的话，表示在两个标记中间
					If Not @error Then
						TrayTip("", "keep moving 3 ", 1, 16)
						If $coord1[0] + 130 >= 790 Or $coord1[1] - 205 >= 580 Then ;判断是否越界，若越界，可能是飞过了
							TrayTip("", "keep moving 3 error", 1, 16)
							exitRoomWithMap()
						Else
							MouseClick("right", $coord1[0] + 130 + Random(5, 10, 1), $coord1[1] - Random(205, 210, 1), Random(1, 3, 1)) ;右边参照
							Sleep(200 + $parm_speeddelay)
							;CheckMove($Char_CheckMoveDelay)
							$coord2 = PixelSearch(10, 20, 350, 300, $biaojicolor1, 30, 1, $title) ; 再次判断是否tp 到房间里了，左上找不到标记了
							If @error Then
								$findflag = 1
								;Sleep(1000)
							Else
					...
					Else
						...
						$coordLeft = PixelSearch(2, 350, 200, 560, $biaojicolor1, 30, 1, $title) ;看到位置后，找到相应位置的地方,
						If Not @error Then ;修正方向
							TrayTip("", "Correct tp..", 1, 16)
					...
					EndIf
				Else
					TrayTip("", "keep Correcting tp。。", 1, 16)
					;Sleep(1000)
					;Return
				EndIf
				$dif = TimerDiff($beginAttackTime)
...
Func isOutTown() ; 在城外
	If isInRoom() = True And findPointColor(125, 595, "28241C") = True Then ;28241C  外
		Return True
	Else
		Return False
	EndIf
...
Func ReturnToTownWithMap()
	Sleep(100)
	Send("{BS}") ;键盘上的的backsapce
	Sleep(2500)
	If isInTown() = False Then ;需要检查是否回到城镇失败了，可能是掉线，或者没有回城卷轴
		TrayTip("", "Back to town False.", 1, 16)
		Sleep(100)
		Return False
	EndIf
	Return True
...
Func bagisfull()
	If isInRoom() = True And $checkbag = 1 And $parm_MoveBox = 1000 Then ;不用这种方式了
		Send("{" & $char_Bag & "}")
		Sleep(100)
		MouseMove(300, 440)
		Sleep(100)
		...
		If isInRoom() And findPointColor(500, 440, "4C4C4C") = True And $emptybag <= $parm_boxqty Then
			...
			If $shutdown = 1 And isInRoom() And $boxisfull = 1 And $emptybag <= 8 Then ;如果用户选择了包裹满了关机,则提示
				Sleep(100)
				exitRoom()
				Sleep(2000)
				TrayTip("", "正在执行关机", 1, 16)
	...
	If isInRoom() = True And $checkbag = 1 And $parm_MoveBox = 2 Then ;启用固定ＫＰ局数的转移
		TrayTip("", "boxisfull" & $boxisfull, 1, 16)
		If $boxisfull = 0 Then ;如果记录仓库已满的表示 =1 ，表示满了，就不用再存仓库里，0 去存仓库
		...
		Else
			;增加一个5局监测一次
			If Mod($movebagcount, 5) = 0 And $movebagcount > 5 Then
				Send("{" & $char_Bag & "}")
				Sleep(500)
				MouseMove(300, 440)
				Sleep(500)
				...
				If $shutdown = 1 And isInRoom() And $boxisfull = 1 And $emptybag <= 4 Then ;如果用户选择了包裹满了关机,则提示
					...
					Return True
				Else
					TrayTip("", "包裹还未满呢，可以继续打几把" & $emptybag, 1, 16)
					Sleep(1000)
					Return False
				EndIf
			EndIf
...
;~ 		EndIf
		...
		$coord = PixelSearch(200, 20, 500, 200, $biaojicolor1, 25, 2, $title)
		If Not @error Then ;找到蓝色， 继续找同一水平行是否有白色
			; 再次检查下方，没有放开才对
			$coord1 = PixelSearch(10, 300, 570, 520, $biaojicolor1, 25, 2, $title)
			If Not @error Then
				Return False
			Else
				Return True
...
Func astisdead()
	TrayTip("", "checking pet if it is dead..", 1, 16)
	Sleep(1)
	If isInRoom() Then
...
Func resumepet()
	...
	If $parm_imageMode = 2 And $dingZhiFlag = 113 Then
		findpath(11301)
		;通过找方块
		$monsterColor1 = "0xA420FC" ;绿色方块 0x18FB01        ；紫色 0xA420FC  红色 0xFC2C00 ，，"0xCE8523" ;橙色
		$monsterColor1_hex = "A420FC"
		$findflag113 = 0
		$begin1 = TimerInit()
		Do
			$tp_Pix = countFirepointRec(10, 5, 700, 540, $monsterColor1, $monsterColor1_hex) ;2  $biaojicolor1    ;0x00FC19, "00FC19"
	...
	If $parm_imageMode = 2 Then
		...
		Do
			...
			If $tp_Pix[0] >= 0 And $tp_Pix[1] >= 0 Then
				...
				$coord = PixelSearch(250, 50, 500, 300, 0xCE8523, 30, 1, $title) ; 在
				If Not @error Then
					;TrayTip("", "准备复活！", 1, 16)
					;Sleep(2000)
					;MouseClick("right", $coord[0], $coord[1], 1);
...
Func a5bianshi()
	...
	If $parm_imageMode = 2 Then
		...
		;通过找方块
		$monsterColor1 = "0xFC2C00" ;绿色方块 0x18FB01        ；紫色 0xA420FC  ,红色 0xFC2C00
...
Func ramclose()
	...
	$ranKp = Random(-4, 4, 1)
	;$ranKp = 0
	If $tfclosecount >= $parm_closetime + 0 Then ; 切换账号后 循环次数也大于规定的
		;此时可以关闭游戏,用循环的下一个账号来登陆
		closeAndWait(0)
	EndIf
...
;~ 	$monsterColor1 = "0xA420FC" ;绿色方块 0x18FB01        ；紫色 0xA420FC
;~ 	$monsterColor1_hex = "A420FC"
...
;~ 	EndIf
	...
	Do
		...
		If $tp_Pix[0] >= 0 And $tp_Pix[1] >= 0 Then
			...
			$coord = PixelSearch(100, 30, 500, 300, 0xCE8523, 30, 1, $title) ; ;点橙色
			If Not @error Then
				TrayTip("", "click to trade！", 1, 16)
				;MouseClick("right", $coord[0], $coord[1], 1);
...
Func child1()
	...
	While ProcessExists($gi_CoProcParent)
		...
		If isInRoom() = True And isOutTown() And $petStatus = 0 And $needResumePetFlag = 0 Then ;站城内部退出
