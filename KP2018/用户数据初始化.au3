...
_GUICtrlListView_DeleteAllItems ( GUICtrlGetHandle ($GUI_ListBox) )
    _SQLite_Open ($SQLite_Data_Path)
    _SQLite_Query(-1, "SELECT * FROM tlog ORDER BY IDs DESC;",$hQuery)
...
_SQLite_QuerySingleRow(-1, "SELECT IDs FROM tlog WHERE IDs = '" & $a & "';", $aRow)
    $Temp = $aRow[0]
If $Temp = "" Then
  _SQLite_Exec(-1, "Insert into tlog (IDs) values ('" & $a & "');")
EndIf
_SQLite_Exec(-1, "UPDATE tlog SET firstdate = '" & $b & "' WHERE IDs = '" & $a & "';")
...
_SQLite_QuerySingleRow(-1, "SELECT * FROM tlog WHERE IDs = '" & $a & "';", $aRow)
    $Temp = $aRow[0]
If $Temp = "" Then
  MsgBox(262208, "查找结果...", "数据库中编号为 [" & $a & "] 的员工记录不存在！")
    Else
  _GUICtrlListView_DeleteAllItems ( GUICtrlGetHandle ($GUI_ListBox) )
        _GUICtrlListView_AddItem($GUI_ListBox, $aRow[0])
        _GUICtrlListView_AddSubItem($GUI_ListBox, _GUICtrlListView_FindInText($GUI_ListBox, $aRow[0]), $aRow[1], 1)
...
Func SQLiteDelete($a)
    _SQLite_Open ($SQLite_Data_Path)
    _SQLite_Exec(-1, "DELETE FROM  tlog WHERE IDs = '" & $a & "';")
    _SQLite_Close ()
